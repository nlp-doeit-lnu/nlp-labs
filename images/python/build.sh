...
function build_image () {
	...
	echo "check if nlp-python:latest exists"
	if [ -z "$(docker images -q $IMAGE:$TAG 2> /dev/null)" ];
	then
		echo "it doesn't exits."
		echo "run docker build."
		
		docker build -t $IMAGE:$TAG $NLP_PYTHON

		echo "nlp-python:latest created."
	else
		echo "image nlp-python:latest already exists."
	fi
}
...
NLP_HOME=$PWD
NLP_PYTHON=$NLP_HOME/images/python
IMAGE=nlp-python
TAG=latest
...
echo "creating script $NLP_PYTHON/run.sh"
...
sed -e "s@%NLP_HOME%@$NLP_HOME@g" \
    ...
    -e "s@%XKEY%@$XKEY@g" \
    -e "s@%TAG%@$TAG@g" \
    $NLP_PYTHON/run.template > $NLP_PYTHON/run.sh

chmod u+x $NLP_PYTHON/run.sh
